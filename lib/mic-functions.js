...
function dateToLocalIsoString(date) {
    const timezineOffset = date.getTimezoneOffset() * 60000; //offset in milliseconds
    return (new Date(date.getTime() - timezineOffset)).toISOString().slice(0, -1);
}
...
/**
 * Convert a list, given as semicolon-separated string, into array.
 * @param {string}  input   String
 * @return {array}          Array of white/black list items, with string or regex elements
 */
function stringConfigListToArray(input) {
    const result = [];
    if(!isLikeEmpty(input)) {
        ...
        for (const lpConfBlacklist of inputArray) {
            // See description of function convertRegexpString().
            result.push(convertRegexpString(lpConfBlacklist));
        }
    }
    return result;
}
 ...
 * if no regex recognized, we keep the string as is.
 ...
 */
function convertRegexpString(input) {
