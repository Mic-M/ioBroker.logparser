...
// Currently not used.
// Also see https://github.com/ioBroker/ioBroker.repositories/pull/741#issuecomment-642248790
// -> "this can produce wrong results. Better to use a getObjectsView for objects of type "instance""
/**
 * Returns an array of all adapter instance state ids in the callback function, or an empty array, 
 * if no adapter instance found (which would be strange).
 * Return value within callback is like ['admin.0', 'alexa2.0']
 * Mic-M // 18/Apr/2020
 * 
 * param  {object}    adapter    The adapter object
 * @return {object}  callback   Callback function with the resulting array
 */
/*
 function getAllAdapterInstanceIds(adapter, callback) {
    adapter.getForeignObjects('system.adapter.*.*.alive', 'state', (err, allObjects)=> {
        if (err) return callback([]);
        const allKeysPrevious = Object.keys(allObjects); // get all keys in array
        const finalStates = [];
        for (const lpObj of allKeysPrevious) {
            // Remove everything after last dot '.', so removes '.alive' of 'system.adapter.adapter-studio.0.alive'
            const allExceptLast = lpObj.substr(0, lpObj.lastIndexOf('.')); 
            // Remove "system.adapter." from beginning and push into array
            finalStates.push(allExceptLast.substr(15));
        }
        return callback(finalStates);
    });
}
*/
 ...
 * @return {array}              Array of list items as regex
 */
function stringConfigListToArray(adapter, filterName, optionTitle, input, addGlobal=false) {
    const result = [];
    if(isLikeEmpty(input)) return [];
    ...
    for (const lpItem of inputArray) {
        ...
        const converted = convertRegexpString(lpItem, addGlobal);
        if (typeof converted == 'string' && converted.startsWith('Regex Error: ')) {
            ...
            adapter.log.warn('Filter "'+ filterName + '", option "' + optionTitle + '":' + converted);
            adapter.log.warn('Therefore, regex in filter "'+ filterName + '", option "' + optionTitle + '" will be ignored.');
        } else {
 ...
 * @param {boolean} [addGlobal=false]   If true and if it is a string, we will add the global flag 'g'
 * @return {RegExp|string}              regexp or string. If Regex error: String starting with 'Regex Error: '
 */
